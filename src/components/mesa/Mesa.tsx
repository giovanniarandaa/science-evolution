...
/**
 * La Mesa de Trabajo: armás el invento pieza por pieza (mecánica central).
 * Genérica y data-driven — recibe un `Process` con sus `pieces`, `scene` y `steps`.
 * Los gestos (friccionar, soplar, enrollar) se hacen IN SITU sobre la escena,
 * sin modales: el proceso ES el juego.
 */
export function Mesa({ process }: { process: Process }) {
  ...
  const W = vbW || 520;
  const H = vbH || 400;
  const posOf = (slot: string) => ({
    left: `${(slots[slot].x / W) * 100}%`,
    top: `${(slots[slot].y / H) * 100}%`,
  });

  // --- piezas ↔ slots ---
  const { slotToPiece, pieceToSlot } = useMemo(() => {
    const s2p: Record<string, string> = {};
    const p2s: Record<string, string> = {};
    for (const st of process.steps) {
      if (st.interaction?.type === "place") {
        s2p[st.interaction.slot] = st.interaction.piece;
        p2s[st.interaction.piece] = st.interaction.slot;
      }
    }
    return { slotToPiece: s2p, pieceToSlot: p2s };
  }, [process]);

  const pieceById = (id: string) => process.pieces?.find((p) => p.id === id);
  const emojiOf = (pieceId: string) => {
    const from = pieceById(pieceId)?.fromElement;
    return (from ? elementsById[from]?.emoji : undefined) ?? "🔩";
  };

  // --- paso activo ---
  const activePlacePiece =
    activeStep?.interaction?.type === "place" ? activeStep.interaction.piece : null;
  const activePlaceSlot =
    activeStep?.interaction?.type === "place" ? activeStep.interaction.slot : null;
  const gesture =
    activeStep?.interaction?.type === "gesture" ? activeStep.interaction.gesture : null;
  ...
  const startDrag = (e: ReactPointerEvent, pieceId: string) => {
    ...
    e.preventDefault();
    setDrag({ piece: pieceId, x: e.clientX, y: e.clientY });
    ...
    const up = (ev: PointerEvent) => {
      ...
      setDrag(null);
      const g = ghostRef.current?.getBoundingClientRect();
      if (
        g &&
        activePlaceSlot &&
        ...
        dispatch({ type: "attempt", attempt: { kind: "place", piece: pieceId, slot: activePlaceSlot } });
      }
    };
  ...
  const [gp, setGp] = useState(0); // progreso del gesto 0..1
  const gpRef = useRef(0);
  const lastRef = useRef<{ x: number; t: number } | null>(null);
  const holdRef = useRef(false);
  const rafRef = useRef<number | null>(null);

  // reset del gesto al cambiar de paso
  useEffect(() => {
    gpRef.current = 0;
    setGp(0);
  ...
  const pieceUsed = (pieceId: string) => state.filledSlots.includes(pieceToSlot[pieceId]);
  ...
  return (
    <div className="min-h-screen w-full bg-stone-100">
      <div className="mx-auto flex w-full max-w-5xl flex-col gap-6 p-4 lg:flex-row lg:items-start">
        ...
        >
          {SceneComp ? (
            ...
            state.filledSlots.map((slot) => (
              <div
                key={slot}
                className="absolute flex -translate-x-1/2 -translate-y-1/2 flex-col items-center rounded-xl border bg-white/90 px-2 py-1 text-2xl shadow"
                style={posOf(slot)}
              >
                {emojiOf(slotToPiece[slot])}
              </div>
            ))
          )}

          {/* ghost del slot activo (dónde soltar) — solo en pasos 'place' */}
          {activePlaceSlot && (
            <div
              ref={ghostRef}
              ...
              style={posOf(activePlaceSlot)}
            >
              acá
            </div>
          )}

          {/* capa de gesto IN SITU (friccionar / soplar / enrollar) */}
          {isGesture && (
            <div
              ...
              onPointerUp={
                gestureKind === "rub" ? onRubUp : gestureKind === "hold" ? endHold : undefined
              }
      ...
      {/* pieza "volando" durante el drag */}
      {drag && (
        <div
          className="pointer-events-none fixed z-50 flex h-12 w-12 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-xl border-2 border-amber-500 bg-white shadow-xl"
